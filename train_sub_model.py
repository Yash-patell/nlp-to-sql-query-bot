...
# train sub-models based on your specific requirements

# Load your data, replace with your dataset path
data = pd.read_csv('./database/train_join.csv') # dataset for training join query model

# data = pd.read_csv('./database/train_order.csv') -- dataset for training order query model

# data = pd.read_csv('./database/train_select.csv') -- dataset for select queries model
...
# Build the model
model = Sequential()
model.add(Embedding(vocab_size_questions, embedding_dim, input_length=max_question_len))
...
# my_model = model.fit(X_train,y_train, epochs=250, batch_size=16, validation_split = 0.1)
...
# Define the model directory to save the model
model_dir = './LSTM_pooling_join'
